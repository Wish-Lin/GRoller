...
from tkinter import Canvas, Event, Misc, Text, Scrollbar, getboolean
...
class TkLineNumbers(Canvas):
    ...
    Dec 2025 Wei-Hsu Lin: Quick mod to add support for scrollbars 
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Dec 2025 Wei-Hsu Lin: Added a few more variables
        self.text_left_margin = 7  # Left margin of text
        self.text_right_margin = 3 # Right margin of text
    ...
    def redraw(self, *_) -> None:
        ...
        for lineno in range(first_line, last_line + 1):
            ...
            """Modified by Wei-Hsu Lin, Dec 2025"""
            # Create the line number
            self.create_text(
                int(self["width"])-self.text_right_margin, # I only use justfy="right"
                dlineinfo[1],
                text=f"{lineno}",
                anchor={"left": "nw", "right": "ne", "center": "n"}[self.justify],
                font=self.textwidget.cget("font"),
...
(a case not covered in mouse_off_screen_scroll) -- Internal use only"""
    ...
    def resize(self) -> None:
        ...
        # Original function is off by 1
        # Original author probably never noticed since it is only relevant after 9999
        # lines in the original function, LOL
        end = str(int(self.textwidget.index("end").split(".")[0])-1)

        # Set the width of the widget to the required width to display the biggest line number
        # Also include the small margin on left and right
        temp_font = Font(font=self.textwidget.cget("font"))
        self.config(width=temp_font.measure(end)+self.text_left_margin+self.text_right_margin)
        # print(temp_font.measure(end)+self.text_left_margin+self.text_right_margin)
...
if __name__ == "__main__":
    ...
    linenums = TkLineNumbers(root, text, justify="right", colors=ttk_theme_colors)
